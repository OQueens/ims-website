...
// loadMarketBucketRates() to anchor the quote on a corroborated MARKET-TYPED v2
// posterior (else leave the cell on its audited curated band). The sim-adapter's
// quote path reads that SAME SPECIALTIES singleton, so once this resolves every
// re-quote reflects the overlay — the dashboard's "static first paint, then live"
// behaviour. The crude legacy loadMarketRates overlay is RETIRED (not called).
//
...
// shared chunk, so loadMarketBucketRates' in-place mutation reaches the adapter's reads.
